	...
	<div class="span5">
		...
		$(document).ready(function(){
			$('#btn1').click(function() {
				...
				$.ajax({
							type:'POST',
						url: '<?php echo site_url("anakan/cetak"); ?>',
						data:{form,outputType},
						success:function(response){
								if(response == '1'){
									window.open('<?php echo base_url(); ?>'+'print/m_anakan_printlist.html','m_anakanlist','height=600,width=800,resizable=1,scrollbars=1, menubar=0');
								}
						},error:function(){
			...
			$('#btn2').click(function() {
				...
				$.ajax({
						...
						success:function(response){
								if(response == '1'){
									window.location=('<?php echo base_url(); ?>'+'print/m_anakan_printlist.xls');
								}
						},error:function(){
							alert('Gagal Cetak Data');
						}
